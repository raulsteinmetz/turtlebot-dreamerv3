...
class Env(Node):
    ...
    def get_reward_and_done(self, obs):
        """
        ...
        # obs = lidar + [distance_to_target, angle_to_target, linear_vel, angular_vel]
        distance = obs[self.lidar] 
        ...
        elif np.min(obs[:self.lidar]) < COLISION_TRESHOLD:
            self.reached = False
            done = True
            reward = -10
    ...
    def step(self, action):
        ...
        obs = self.get_state(action[0], action[1])

        reward, done = self.get_reward_and_done(obs)
